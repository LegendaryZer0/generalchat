...
public class UserServiceImpl implements UserService {
  ...
  public User addUser(User user) {
    try {
      ...
      log.info(
          "user repo log info by email {},", userRepository.getUserByEmail(user.getEmail()).get());
      String confirmMail =
          mailsGenerator.getEmailforConfirm(
              serverUrl, userRepository.getUserByEmail(user.getEmail()).get().getId());
  ...
  public Integer findCountOfDailyChatedUsers() {
    return userRepository.findDailyChatedUsers().stream()
        .map(User::getId)
  ...
  public User updateUser(User user) {
    ...
    if (user.getPassword()
        .equals(userRepository.getUserByEmail(user.getEmail()).get().getPassword())) {
