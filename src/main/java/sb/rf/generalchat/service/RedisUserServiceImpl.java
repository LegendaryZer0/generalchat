...
public class RedisUserServiceImpl implements RefreshTokenService,TokenAuthenticateService {
    ...
    public Pair<String, String> getRefreshAccessTokensWithRotation(String oldRefreshToken, String accessToken, String deviceName) {
        ...
        String newAccessToken = apiAuthService.generateAccessToken(getExpiredTokenUsername(accessToken));
    ...
    public void authenticateUser(String accessToken){
        ...
        log.info("extracted authority {}",jwtUtil.extractClaim(accessToken,claims -> ((LinkedHashMap<String, String>)claims.get("role")).values().stream().findFirst()).toString());
        tokenAuthentication.setAuthority(jwtUtil.extractClaim(accessToken,claims -> ((LinkedHashMap<String, String>)claims.get("role")).values().stream().findFirst().get()));
