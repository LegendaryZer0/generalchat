...
public class NamesValidator implements ConstraintValidator<ValidPassName, Object> {
    ...
    public void initialize(ValidPassName constraintAnnotation) {
        this.nicknamePropertyName = constraintAnnotation.nickname(); // название поля для name -> firstName
        this.passwordPropertyName = constraintAnnotation.password(); // название поля для surname -> lastName

    }

    @Override
    public boolean isValid(Object value, ConstraintValidatorContext context) {
        Object name = new BeanWrapperImpl(value).getPropertyValue(nicknamePropertyName); //получили конкретные значения
