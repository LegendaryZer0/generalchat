let main_id_from;
let main_id_to;
let testJson;
let socket;
let url;
let data;

let xhr = new XMLHttpRequest();
$(document).ready(function () {
    $('#action_menu_btn').click(function () {
        $('.action_menu').toggle();
    });
    $('#message-send-btn').click(function sendMessage() {
//Todo считать сообщения с полей файлов, и если там что-то есть, отправить сообщение с измененным типом
        let result;
        let type;
        console.log("finding alternative way to src value")
        result = $('#image-preview__image').attr('src');
        console.log(result)

        if (result !== undefined && result !== null &&result !=='' && result!==' ') {
            console.log("trying insert the result")
            var token = $("meta[name='_csrf']").attr("content");
            var header = $("meta[name='_csrf_header']").attr("content");
            console.log("отправляемый токен и header")
            console.log(token)
            console.log(header)
            let data = {
                from: main_id_from,
                to: main_id_to,
                message: result,
                time: new Date(+new Date()),
                type: 'DOCUMENT',
            };
            console.log(result);
            console.log('data to send ' + data.toString());
            //    socket.send(JSON.stringify(data));//todo ajax


            $.ajax({
                url: "sendDocument",
                type: 'POST',
                contentType: "application/json;charset=utf-8",
                data: JSON.stringify(data),
                dataType: 'json', // added data type

                beforeSend: function (xhr) {
                    xhr.setRequestHeader(header, token);
                    console.log("Дата:")
                    console.log(this.data)
                    //nothing needed here
                    //usually you can display a loading message
                },
                success: function (res) {
                    console.log(res);
                    //     parsedRes =JSON.parse(JSON.stringify(res));
                    //   console.log(parsedRes)

                }
            });


            appendPhotoMessageAsUser(result);
            let previewContainer = document.getElementById('image-preview');
            let previewImage = document.getElementById('image-preview__image');
            $('#image-preview__image').attr('src',null)
            previewContainer.style.display = null;
            previewImage.style.display = null;
            result = null;
            return;


        }

        let val = $('#message-input').val();
        $('#message-input').val("");

        let data = {
            from: main_id_from,
            to: main_id_to,
            message: val,
            time: new Date(+new Date()),
            type: 'STRING'
        };

        /*    console.log(JSON.stringify(data));
            console.log("Отправляю этот json");
            console.log(data);
            console.log(socket);*/

        console.log("Sended data:")
        console.log(data);
        if(val!==''){
            socket.send(JSON.stringify(data));
        }
        console.log("Try to send test stomp messgae")


        console.log("Disconnected from stomp")
        console.log(val)
        if (val.includes("youtube.com/watch?v=")) {
            val.split("=")[1];
            console.log(val.split("="))
            console.log("NEW SPLIT")
            console.log(val.split("youtube.com/watch?v=")[1].split("&")[0])

            appendMessage(val + youtubeViewWrapper(val.split("youtube.com/watch?v=")[1].split("&")[0]))

        } else if (val.includes("!!НАЙДИ")) {
            console.log(val)
            console.log(val.split("!!НАЙДИ"))
            console.log(val.split("!!НАЙДИ")[1])
            $.ajax({
                url: "getVideosYoutube/" + val.split("!!НАЙДИ")[1],
                type: 'GET',
                dataType: 'json', // added data type
                success: function (res) {
                    console.log(res);
                    //     parsedRes =JSON.parse(JSON.stringify(res));
                    //   console.log(parsedRes)
                    for (i in res['items']) {
                        console.log(i)
                        //   console.log(i['id']['videoId'])
                        console.log(res['items'][i]['id']['videoId'])
                        appendMessage(youtubeViewWrapper(res['items'][i]['id']['videoId']))
                    }
                }
            });
        } else {
            if(val!==''){
                appendMessage(val)

            }



        }


    });


});


function appendMessage(messageValue, date = new Date(+new Date())) {
    $('#messageBox').append(" <div class=\"d-flex justify-content-start mb-4\">\n" +
        "                        <div class=\"img_cont_msg\">\n" +
        "                            <img src=\"https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg\"\n" +
        "                                 class=\"rounded-circle user_img_msg\">\n" +
        "                        </div>\n" +
        "                        <div class=\"msg_cotainer\">\n" +
        "                            " + messageValue + "  \n" +
        "                            <span class=\"msg_time\"> " + date + "</span>\n" +
        "                        </div>\n" +
        "                    </div>")

}

function appendMessageAsAnotherUser(messageValue, date = new Date(+new Date())) {
    $('#messageBox').append("<div class=\"d-flex justify-content-end mb-4\">\n" +
        "                        <div class=\"msg_cotainer_send\">\n" +
        "                            " + messageValue + " \n" +
        "                            <span class=\"msg_time_send\"> " + date + "</span>\n" +
        "                        </div>\n" +
        "                        <div class=\"img_cont_msg\">\n" +
        "                            <img src=\"data:image/jpeg;base64,\"\n" +
        "                                 class=\"rounded-circle user_img_msg\">\n" +
        "                        </div>\n" +
        "                    </div>")

}


function appendPhotoMessageAsUser(messageValue, date = new Date(+new Date())) {
    $('#messageBox').append(" <div class=\"d-flex justify-content-start mb-4\">\n" +
        "                        <div class=\"img_cont_msg\">\n" +
        "                            <img src=\"https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg\"\n" +
        "                                 class=\"rounded-circle user_img_msg\">\n" +
        "                        </div>\n" +
        "                        <div class=\"msg_cotainer\">\n" +
        "                            " + "<img src=" + messageValue + " />" + "  \n" +
        "                            <span class=\"msg_time\"> " + date + "</span>\n" +
        "                        </div>\n" +
        "                    </div>")

}

function appendPhotoMessageAsAnotherUser(messageValue, date = new Date(+new Date())) {
    $('#messageBox').append("<div class=\"d-flex justify-content-end mb-4\">\n" +
        "                        <div class=\"msg_cotainer_send\">\n" +
        "                            " + "<img src=" + messageValue + " />" + " \n" +
        "                            <span class=\"msg_time_send\"> " + date + "</span>\n" +
        "                        </div>\n" +
        "                        <div class=\"img_cont_msg\">\n" +
        "                            <img src=\"data:image/jpeg;base64,\"\n" +
        "                                 class=\"rounded-circle user_img_msg\">\n" +
        "                        </div>\n" +
        "                    </div>")

}


function changeHead(n) {
    $('.active').click(function () {
        console.log("Начато общение с " + n);
        $('#chat-right-header').text('Chat with ' + n);

    })


}


function changeChat(id_from, id_to, nick) {
    if (socket !== undefined) {
        console.log("Закрываю сокет")
        socket.close();
    }
    // let uuidUrl =getUrl(id_from,id_to);


    main_id_from = id_from;
    main_id_to = id_to;
    console.log("Получаю сокет")
    socket = new WebSocket("ws://localhost:8080/chat/chatendpoint");
    changeHead(nick);
    console.log("Начинаю подгрузку сообщений")
    loadMesages(id_from, id_to);

    /*    let s = hashCode(id_from + id_to + "")*/
    /*console.log(s);*/

    /*console.log("ws://localhost:8080/LabWork_war/chat/" + url);*/
    socket.onopen = function () {

    };

    socket.onclose = function (event) {
        console.log("ВебСокет был закрыт")
        if (event.wasClean) {
            console.log("Additionaly, ВебСокет был success закрыт")
        } else {
            // например, "убит" процесс сервера
        }

    };

    socket.onmessage = function (message) {
        /* console.log("some liitle parse")*/
        var a = JSON.parse(message.data);

        /*
                         console.log("message");
                         console.log(message.data);
                         console.log("Json parse strigify");*/
        json = JSON.parse(message.data,);
        console.log("json  ");
        console.log(json);
        console.log("json strigify")
        console.log(JSON.stringify(message.data,));
        messageFromAnotherUser = json['message'];
        if (messageFromAnotherUser.includes("!!НАЙДИ")) {

            $.ajax({
                url: "getVideosYoutube/" + messageFromAnotherUser.split("!!НАЙДИ")[1],
                type: 'GET',
                dataType: 'json', // added data type
                success: function (res) {
                    console.log(res);
                    //     parsedRes =JSON.parse(JSON.stringify(res));
                    //   console.log(parsedRes)
                    for (i in res['items']) {
                        console.log(i)
                        //   console.log(i['id']['videoId'])
                        console.log(res['items'][i]['id']['videoId'])
                        appendMessageAsAnotherUser(youtubeViewWrapper(res['items'][i]['id']['videoId']), json['date'])

                    }
                }
            });


        } else {
            if (messageFromAnotherUser.includes("youtube.com/watch?v=")) {
                messageFromAnotherUser = messageFromAnotherUser + youtubeViewWrapper(messageFromAnotherUser.split("youtube.com/watch?v=")[1].split("&")[0])
            }
            if (json['type'] === 'DOCUMENT') {
                console.log("That's document!")
                console.log(json['message'])
                $.ajax({
                    url: "getDocument/" + json['message'],
                    type: 'GET',
                  //  dataType: 'json', // added data type
                    success: function (res) {
                        console.log("result of get document")
                        console.log(res);
                        appendPhotoMessageAsAnotherUser(res)
                        //     parsedRes =JSON.parse(JSON.stringify(res));
                        //   console.log(parsedRes)

                    }
                });
                return;
            }

            appendMessageAsAnotherUser(messageFromAnotherUser, json['date'])


        }


    };

    socket.onerror = function (error) {
        console.log("ОШИБКА")
        console.log(error)
        console.log(error.message)
        console.log(`[error] ${error.message}`);
    };

}


function loadMesages(id_from, id_to) {
    var data = {
        id_to: id_to,
        id_from: id_from

    };
    var token = $("meta[name='_csrf']").attr("content");
    var header = $("meta[name='_csrf_header']").attr("content");
    console.log("Prepered data to send from load Messages")
    console.log(data)
    $.ajax({

        type: 'POST',
        url: 'getInfo',
        contentType: "application/json",
        data: JSON.stringify(data),
        beforeSend: function (xhr) {
            xhr.setRequestHeader(header, token);
        },


        success: function (data) {

            $('#messageBox').html("");
            console.log("Пришедшие сырые сообщения ")
            console.log(data)
            console.log("Пробую выполнить stringify ")
            console.log(JSON.stringify(data))
            var json = (JSON.stringify(data));
            testJson = json[0];
            console.log("Пришедшие сообщения  ")
            console.log(json)
            console.log("Пришедшие сообщения json[0]")
            console.log(testJson)
            $.each(data, function (index, message) {


                if (message['message'].includes("!!НАЙДИ")) {

                    $.ajax({
                        url: "getVideosYoutube/" + message['message'].split("!!НАЙДИ")[1],
                        type: 'GET',
                        dataType: 'json', // added data type
                        success: function (res) {
                            console.log(res);
                            //     parsedRes =JSON.parse(JSON.stringify(res));
                            //   console.log(parsedRes)
                            for (i in res['items']) {
                                console.log(i)
                                //   console.log(i['id']['videoId'])
                                console.log(res['items'][i]['id']['videoId'])
                                if (message['from'] === id_from) {
                                    console.log(message['message']);

                                    appendMessage(youtubeViewWrapper(res['items'][i]['id']['videoId']), message['date'])


                                } else {
                                    console.log("печатаю как иного юзера")
                                    console.log(message['from'])
                                    appendMessageAsAnotherUser(youtubeViewWrapper(res['items'][i]['id']['videoId']), message['date'])

                                }
                            }
                        }
                    });
                } else {

                    final_message = message['message']
                    if (final_message.includes("youtube.com/watch?v=")) {
                        final_message = final_message + youtubeViewWrapper(final_message.split("youtube.com/watch?v=")[1].split("&")[0])
                    }


                    if (message['from'] === id_from) {


                        if (message['type'] === 'DOCUMENT') {
                            console.log("DOcument from this user")
                            $.ajax({
                                url: "getDocument/" + message['message'],
                                type: 'GET',
                               // dataType: 'json', // added data type

                                beforeSend: function (xhr) {
                                    xhr.setRequestHeader(header, token);
                                    //nothing needed here
                                    //usually you can display a loading message
                                },
                                success: function (res) {
                                    console.log(res);
                                    appendPhotoMessageAsUser(res, message['date'])
                                    //     parsedRes =JSON.parse(JSON.stringify(res));
                                    //   console.log(parsedRes)

                                },
                                error: function(xhr, textStatus, error){
                                    console.log(xhr.statusText);
                                    console.log(textStatus);
                                    console.log(error);
                                }
                            });
                        } else {
                            appendMessage(final_message, message['date'])
                        }


                    } else {
                        if (message['type'] === 'DOCUMENT') {
                            console.log("DOcument from another user")
                            $.ajax({
                                url: "getDocument/" + message['message'],
                                type: 'GET',
                           //     dataType: 'json', // added data type

                                beforeSend: function (xhr) {
                                    xhr.setRequestHeader(header, token);
                                    console.log("Дата:")

                                    //nothing needed here
                                    //usually you can display a loading message
                                },
                                success: function (res) {
                                    console.log(res);
                                    appendPhotoMessageAsAnotherUser(res, message['date'])
                                    //     parsedRes =JSON.parse(JSON.stringify(res));
                                    //   console.log(parsedRes)

                                },
                                error: function(xhr, textStatus, error){
                                    console.log(xhr.statusText);
                                    console.log(textStatus);
                                    console.log(error);
                                }
                            });
                        } else {
                            appendMessageAsAnotherUser(final_message, message['date'])
                        }


                    }


                }


            })

        },

    });

}


function youtubeViewWrapper(videoID) {
    return "<iframe width=\"560\" height=\"315\" src=\"https://www.youtube.com/embed/" + videoID + "\" title=\"YouTube video player\" frameborder=\"0\" allow=\"accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture\" allowfullscreen></iframe>"
}







